...
dev tun
proto tcp
remote "www.bug.com" 1194
http-proxy xxxxxxxxx 8888
resolv-retry infinite
route-method exe
nobind
persist-key
...
sed -i $MYIP2 /etc/openvpn/client-ohp-1194.ovpn;
...
echo '<ca>' >> /etc/openvpn/client-ohp-1194.ovpn
cat /etc/openvpn/server/ca.crt >> /etc/openvpn/client-ohp-1194.ovpn
echo '</ca>' >> /etc/openvpn/client-ohp-1194.ovpn

# Copy config OpenVPN client ke home directory root agar mudah didownload ( OHP 1194 )
cp /etc/openvpn/client-ohp-1194.ovpn /home/vps/public_html/client-ohp-1194.ovpn
